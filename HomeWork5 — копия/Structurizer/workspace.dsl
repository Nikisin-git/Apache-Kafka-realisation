    ...
    model {
        ...
        companion_search = softwareSystem "Сервис поиска попутчиков" {
            ...
            kafka = container "Kafka" {
                ...
                -> Route_API_db "Извлечение сообщений и помещение в базу данных" "Mongo protocol"
            }

            route_writer_service = container "route_write_service" {
                technology "Python"
                description "Публикует маршруты в Kafka и сохраняет их в MongoDB"
                -> kafka "Публикует маршруты и подписывается на них" "Kafka protocol"
            }

            auth_API_db = container "База данных аутентификации" {
                tags "Database"
                technology "PostgreSQL Database"
                description "Хранение хэшированных токенов и данных о пользователях"
            }
            ...
            Trip_API = container "API поездок" {
                ...
                -> route_writer_service "Публикует события о новых маршрутах" "Kafka"
        ...
        }  // конец softwareSystem companion_search
